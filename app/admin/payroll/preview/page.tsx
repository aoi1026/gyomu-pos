            ...
            >
                      ...
                      {expandedDailyRows[row.user_id] && dailyRowsData[row.user_id] && (
                        <>
                          ...
                          <tr className="bg-blue-50 border-t border-blue-200">
                            <td colSpan={22 + payrollCategories.length} className="px-3 py-1.5">
                              <div className="flex items-center justify-between gap-2">
                                <span className="text-xs font-semibold text-blue-700">▼ 日別内訳: {row.name}</span>
                                <div className="flex items-center gap-1">
                                  <Button
                                    variant="outline"
                                    size="sm"
                          ...
                          {/* 日別データ行（外側テーブルに直接挿入して列幅を一致させる） */}
                          {dailyRowsData[row.user_id].map((dailyRow: any, dailyIdx: number) => (
                            <tr key={`dr-${row.user_id}-${dailyIdx}`} className="border-t bg-blue-50/40 hover:bg-blue-50/70 transition-colors">
                              <td className="p-2 sm:p-3 whitespace-nowrap sticky left-0 bg-[#eff6ff] z-10 border-r border-gray-200 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.08)]">
                                <div className="font-medium text-xs sm:text-sm">{formatDisplayDate(dailyRow.date)}</div>
                              </td>
                              <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                                <div className="text-xs sm:text-sm text-gray-700">{formatHours(dailyRow.basic_hours)}</div>
                              </td>
                              <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap border-l-2 border-gray-300">{formatCurrency(dailyRow.paid_price ?? 0)}</td>
                              <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">{formatCurrency(dailyRow.pickup_yen ?? 0)}</td>
                              <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">{formatCurrency(dailyRow.hairmake_yen ?? 0)}</td>
                              <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">{formatCurrency(dailyRow.rental_yen ?? 0)}</td>
                              <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap border-r-2 border-gray-300">{formatCurrency(dailyRow.other_deduct_yen ?? 0)}</td>
                              <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap border-l-2 border-gray-400">{formatCurrency(dailyRow.penalty_yen ?? 0)}</td>
                              <td className="p-2 sm:p-3 text-center font-semibold text-xs sm:text-sm whitespace-nowrap">{formatCurrency(dailyRow.deduction_yen ?? 0)}</td>
                              <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">{formatCurrency(dailyRow.hourly_price ?? 0)}</td>
                              ...
                              <td className="p-2 sm:p-3 text-center whitespace-nowrap border-l-2 border-gray-300">
                                <div className="leading-tight">
                                  ...
                                  <div className="font-semibold text-xs sm:text-sm">{formatCurrency(dailyRow.main_nomination_fee || 0)}</div>
                                </div>
                              </td>
                              <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                                <div className="leading-tight">
                                  <div className="text-[10px] text-gray-600">{Number(dailyRow.main_nomination_extension_count || 0)}</div>
                                  <div className="font-semibold text-xs sm:text-sm">{formatCurrency(dailyRow.main_nomination_extension_fee || 0)}</div>
                                </div>
                              </td>
                              <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                                <div className="leading-tight">
                                  <div className="text-[10px] text-gray-600">{Number(dailyRow.inside_nomination_count || 0)}</div>
                                  <div className="font-semibold text-xs sm:text-sm">{formatCurrency(dailyRow.inside_nomination_fee || 0)}</div>
                                </div>
                              </td>
                              <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                                <div className="leading-tight">
                                  <div className="text-[10px] text-gray-600">{Number(dailyRow.inside_nomination_extension_count || 0)}</div>
                                  <div className="font-semibold text-xs sm:text-sm">{formatCurrency(dailyRow.inside_nomination_extension_fee || 0)}</div>
                                </div>
                              </td>
                              <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                                <div className="leading-tight">
                                  <div className="text-[10px] text-gray-600">{Number(dailyRow.together_nomination_count || 0)}</div>
                                  <div className="font-semibold text-xs sm:text-sm">{formatCurrency(dailyRow.together_nomination_fee || 0)}</div>
                                </div>
                              </td>
                              {payrollCategories.map((c) => (
                                <td key={c.id} className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">
                          ...
                          <tr className="border-t-2 border-blue-300 bg-blue-100 font-semibold">
                            <td className="p-2 sm:p-3 text-xs sm:text-sm sticky left-0 bg-blue-100 z-10 border-r border-gray-200 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.08)]">合計</td>
                            ...
                            <td className="p-2 sm:p-3 text-center whitespace-nowrap border-l-2 border-gray-300">
                              <div className="leading-tight">
                                ...
                                <div className="text-xs sm:text-sm">{formatCurrency(dailyRowsData[row.user_id].reduce((sum: number, r: any) => sum + Number(r.main_nomination_fee || 0), 0))}</div>
                              </div>
                            </td>
                            <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                              <div className="leading-tight">
                                ...
                                <div className="text-xs sm:text-sm">{formatCurrency(dailyRowsData[row.user_id].reduce((sum: number, r: any) => sum + Number(r.main_nomination_extension_fee || 0), 0))}</div>
                              </div>
                            </td>
                            <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                              <div className="leading-tight">
                                ...
                                <div className="text-xs sm:text-sm">{formatCurrency(dailyRowsData[row.user_id].reduce((sum: number, r: any) => sum + Number(r.inside_nomination_fee || 0), 0))}</div>
                              </div>
                            </td>
                            <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                              <div className="leading-tight">
                                ...
                                <div className="text-xs sm:text-sm">{formatCurrency(dailyRowsData[row.user_id].reduce((sum: number, r: any) => sum + Number(r.inside_nomination_extension_fee || 0), 0))}</div>
                              </div>
                            </td>
                            <td className="p-2 sm:p-3 text-center whitespace-nowrap">
                              <div className="leading-tight">
                                ...
                                <div className="text-xs sm:text-sm">{formatCurrency(dailyRowsData[row.user_id].reduce((sum: number, r: any) => sum + Number(r.together_nomination_fee || 0), 0))}</div>
                              </div>
                            </td>
                            {payrollCategories.map((c) => (
                              <td key={c.id} className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">
                                <div className="leading-tight">
                                  ...
                                  <div>
                                    {formatCurrency(dailyRowsData[row.user_id].reduce((sum: number, r: any) => {
                            ...
                            <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">{formatCurrency(dailyRowsData[row.user_id].reduce((sum: number, r: any) => sum + Number(r.total_pay_yen || 0), 0))}</td>
                            <td className="p-2 sm:p-3 text-center font-bold text-xs sm:text-sm whitespace-nowrap border-l border-gray-200">
                          ...
                          <tr className="border-b-2 border-blue-200">
                            <td colSpan={22 + payrollCategories.length} className="h-0.5 bg-blue-50" />
                          </tr>
                        </>
                      )}
                      </React.Fragment>
                ...
                <tfoot className="bg-gray-50">
                  <tr className="border-t-2 border-gray-300 font-semibold">
                    <td className="p-2 sm:p-3 sticky left-0 bg-gray-50 z-10 border-r border-gray-200 shadow-[2px_0_4px_-2px_rgba(0,0,0,0.08)]">合計</td>
                    <td className="p-2 sm:p-3 text-center text-xs sm:text-sm whitespace-nowrap">
