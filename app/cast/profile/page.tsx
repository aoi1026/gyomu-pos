...
export default function CastProfilePage() {
  ...
  const handleSaveEmail = async () => {
    ...
    setSavingEmail(true);
    try {
      const res = await fetch(`/api/casts/${user.id}`, {
        method: 'PUT',
      ...
      if (typeof window !== 'undefined' && user?.role === 'cast') {
        localStorage.setItem('cast_auth', JSON.stringify(updated));
      }
      setUser(updated);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-lg sm:text-xl font-bold text-gray-900">個人情報管理</h1>
                  <p className="text-xs sm:text-sm text-gray-500">メール・パスワードの更新</p>
                </div>
              </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center"><Mail className="w-5 h-5 mr-2" />メールアドレスの更新</CardTitle>
            </CardHeader>
