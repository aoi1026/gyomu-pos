...
export default function TableLoginPage() {
  ...
  // 既にログイン済みなら自動でテーブル一覧へ遷移
  useEffect(() => {
    try {
      const stored = typeof window !== 'undefined' ? localStorage.getItem('table_auth') : null;
      if (stored) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/auth/table-login', {
        method: 'POST',
      ...
      <div className="w-full md:w-1/2 flex flex-col bg-white relative min-h-screen">
        ...
        <div className="flex-1 flex flex-col justify-center items-center px-6 sm:px-12 lg:px-16 -mt-20">
          <div className="w-full max-w-sm text-center mb-8">
            ...
            <p className="text-sm text-gray-500 mt-1">テーブル専用ログイン</p>
          </div>
